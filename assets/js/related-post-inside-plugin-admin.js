(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltrpsi-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltrpsi-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltrpsi-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltrpsi-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTRPSICORE.admin_ajax,
        type: "POST",
        data: {
            action: "jlt_rpsi_recommended_activate_plugin",
            file: file,
            nonce: JLTRPSICORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jlt_rpsi_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTRPSICORE.recommended_nonce
        },
