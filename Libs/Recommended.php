...
if ( ! class_exists( 'Recommended' ) ) {
	...
	class Recommended {
		...
		public function __construct( $menu_order = 99 ) {
			...
			add_action( 'wp_ajax_jlt_rpsi_recommended_upgrade_plugin', array( $this, 'jlt_rpsi_recommended_upgrade_plugin' ) );
			add_action( 'wp_ajax_jlt_rpsi_recommended_activate_plugin', array( $this, 'jlt_rpsi_recommended_activate_plugin' ) );
		}
		...
		public function render_recommended_plugins() { ?>
			<div class='jltrpsi-recommended-wrapper'>
				<?php $this->header(); ?>
				<?php $this->body(); ?>
			</div>
			<?php
		...
		public function header() {
			?>
			<div class='jltrpsi-recommended-header'>
				<div class='jltrpsi-recommended-title'>
					<h2>
						<?php echo esc_html__( 'Recommended Plugins', 'related-post-inside-plugin' ); ?>
					</h2>
				</div>
				<div class='jltrpsi-recommended-menu'>
					<div class="wp-filter">
						...
						<form class="search-form jltrpsi-search-plugins mr-0" method="get">
							<input type="hidden" name="tab" value="search">
							<label class="screen-reader-text" for="search-plugins">
								<?php echo esc_html__( 'Search Plugins', 'related-post-inside-plugin' ); ?>
							</label>
							<input type="search" name="s" id="search-plugins" value="" class="wp-filter-search" placeholder="<?php echo esc_html__( 'Search plugins...', 'related-post-inside-plugin' ); ?>">
							<input type="submit" id="search-submit" class="button hide-if-js" value="<?php echo esc_html__( 'Search Plugins', 'related-post-inside-plugin' ); ?>">
						</form>
					</div>
				</div>
			</div>
					...
					<div class="plugin-card-bottom">
						<div class="column-downloaded">
							<span class="plugin-status">
								<?php
								echo esc_html__( 'Status:', 'related-post-inside-plugin' );

								if ( 'install' === $install_status['status'] ) {
									?>
									<span class="plugin-status-not-install" data-plugin-url="<?php echo esc_attr( $plugin['download_link'] ); ?>"><?php echo esc_html__( 'No Installed', 'related-post-inside-plugin' ); ?></span>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										?>
										<span class="plugin-status-active">
											<?php echo esc_html__( 'Active', 'related-post-inside-plugin' ); ?>
										</span>
										<?php
									} else {
										?>
										<span class="plugin-status-inactive" data-plugin-file="<?php echo esc_attr( esc_attr( $install_status['file'] ) ); ?>">
											<?php echo esc_html__( 'Inactive', 'related-post-inside-plugin' ); ?>
										</span>
										<?php
									}
								...
								if ( 'install' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="install-now button button-primary" data-install-url="<?php echo esc_attr( $plugin['download_link'] ); ?>">
											<?php echo esc_html__( 'Install Now', 'related-post-inside-plugin' ); ?>
										</button>
									</li>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="update-now button" data-plugin="<?php echo esc_attr( $install_status['file'] ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-update-url="<?php echo esc_attr( $install_status['url'] ); ?>">
											<?php echo esc_html__( 'Update Now', 'related-post-inside-plugin' ); ?>
										</button>
									</li>
									<?php
								} elseif ( ( 'latest_installed' === $install_status['status'] ) || ( 'newer_installed' === $install_status['status'] ) ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										...
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Activated', 'related-post-inside-plugin' ); ?>
											</button>
										</li>
										<?php
									} elseif ( current_user_can( 'activate_plugin', $install_status['file'] ) ) {
										...
										<button class="button activate-now" data-plugin-file="<?php echo esc_attr( $install_status['file'] ); ?>">
											<?php echo esc_html__( 'Activate', 'related-post-inside-plugin' ); ?>
										</button>
										<?php
									} else {
										?>
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Installed', 'related-post-inside-plugin' ); ?>
											</button>
										</li>
										<?php
									}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_rpsi_recommended_activate_plugin() {
			try {
				if ( isset( $_POST['file'] ) ) {
					$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';

					if ( ! wp_verify_nonce( $nonce, 'jlt_rpsi_recommended_nonce' ) ) {
						wp_send_json_error( array( 'mess' => __( 'Nonce is invalid', 'related-post-inside-plugin' ) ) );
					}
					...
					wp_send_json_success(
						array(
							'mess' => __( 'Activate success', 'related-post-inside-plugin' ),
						)
					);
				}
			} catch ( \Exception $ex ) {
				wp_send_json_error(
					array(
						'mess' => __( 'Error exception.', 'related-post-inside-plugin' ),
						array(
							'error' => $ex,
						),
					)
				...
				wp_send_json_error(
					array(
						'mess' => __( 'Error.', 'related-post-inside-plugin' ),
						array(
							'error' => $ex,
						),
					)
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jlt_rpsi_recommended_upgrade_plugin() {
			try {
				require_once ABSPATH . 'wp-admin/includes/plugin-install.php';
					...
					if ( 'install' === $type ) {
						...
						if ( $plugin_data && ! is_wp_error( $plugin_data ) ) {
							...
							if ( is_wp_error( $active_plugin ) ) {
								...
								wp_send_json_success(
									array(
										'mess' => __( 'Install success', 'related-post-inside-plugin' ),
									)
								);
							}
						} else {
						...
						if ( is_wp_error( $result ) ) {
							...
							wp_send_json_success(
								array(
									'mess'   => __( 'Update success', 'related-post-inside-plugin' ),
									'active' => $is_active,
								)
							);
						}
