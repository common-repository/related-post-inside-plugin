...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'jlt_rpsi_display_notice', array( $this, 'display_notice' ), 10, 2 );
		add_action( 'jlt_rpsi_display_popup', array( $this, 'display_popup' ), 10, 2 );

		add_action( 'wp_ajax_jlt_rpsi_notification_action', array( $this, 'notification_action' ) );
	}
	...
	public function notification_action() {
		check_ajax_referer( 'jlt_rpsi_notification_nonce' );
	...
	public function setup_notifications_by_type( $type ) {
		...
		// Block if necessary .
		$notification_last_fired = get_option( "jlt_rpsi_{$type}_last_interact" );
		...
		do_action( "jlt_rpsi_display_{$type}", $notification, $trigger_time );
	}
	...
	public function display_popup( $popup, $trigger_time ) {
		...
		<div class="jltrpsi-popup" id="jltrpsi-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1">

			<div class="jltrpsi-popup-overlay"></div>

			<div class="jltrpsi-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltrpsi-popup-color: <?php echo esc_attr( $popup->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltrpsi-popup-modal-footer">
					...
					<div class="jltrpsi-popup-countdown" style="display: none;">
						<span class="jltrpsi-popup-countdown-text"><?php echo esc_html__( 'Deal Ends In', 'related-post-inside-plugin' ); ?></span>
						<div class="jltrpsi-popup-countdown-time">
							<div>
								<span data-counter="days">00</span>
								<span><?php echo esc_html__( 'Days', 'related-post-inside-plugin' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="hours">00</span>
								<span><?php echo esc_html__( 'Hours', 'related-post-inside-plugin' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="minutes">00</span>
								<span><?php echo esc_html__( 'Minutes', 'related-post-inside-plugin' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="seconds">00</span>
								<span><?php echo esc_html__( 'Seconds', 'related-post-inside-plugin' ); ?></span>
							</div>
						</div>
					</div>

					<!-- button  -->
					<a class="jltrpsi-popup-button" target="_blank" href="<?php echo esc_url( $popup->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $popup->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			function jlt_rpsi_popup_action(evt, $this, $action_type) {

				evt.preventDefault();

				$this.closest('.jltrpsi-popup').fadeOut(200);
			...
			// Notice Dismiss
			jQuery('body').on('click', '.jltrpsi-popup .popup-dismiss', function(evt) {
				jlt_rpsi_popup_action(evt, jQuery(this), 'dismiss');
			});

			// Notice Disable
			jQuery('body').on('click', '.jltrpsi-popup .popup-disable', function(evt) {
				jlt_rpsi_popup_action(evt, jQuery(this), 'disable');
			});
		</script>
